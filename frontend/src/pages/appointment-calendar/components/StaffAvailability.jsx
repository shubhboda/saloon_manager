  ...
  return (
    <div className="bg-card border border-border rounded-lg p-4 salon-shadow-card salon-transition-layout">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-foreground font-sans">Staff Availability</h3>
        <div className="flex items-center space-x-4 text-xs">
          <div className="flex items-center space-x-1">
            <div className="w-2 h-2 bg-success rounded-full salon-transition-micro"></div>
            <span className="text-muted-foreground font-sans">Available</span>
          </div>
          <div className="flex items-center space-x-1">
            <div className="w-2 h-2 bg-warning rounded-full salon-transition-micro"></div>
            <span className="text-muted-foreground font-sans">Busy</span>
          </div>
          <div className="flex items-center space-x-1">
            <div className="w-2 h-2 bg-accent rounded-full salon-transition-micro"></div>
            <span className="text-muted-foreground font-sans">Break</span>
          </div>
        </div>
      </div>
      <div className="space-y-3">
        {staffMembers?.map(staff => {
          ...
          return (
            <div
              key={staff?.id}
              className="flex items-center justify-between p-3 rounded-lg border border-border hover:bg-muted cursor-pointer salon-transition-micro salon-hover-scale salon-shadow-card"
            ...
            >
              <div className="flex items-center space-x-3">
                <div className="relative">
                  ...
                  <div className={`absolute -bottom-1 -right-1 w-4 h-4 ${getStatusColor(status)} rounded-full border-2 border-card salon-transition-micro`}></div>
                </div>

                <div>
                  <h4 className="font-medium text-foreground font-sans">{staff?.name}</h4>
                  <p className="text-sm text-muted-foreground">{staff?.role}</p>
                </div>
              </div>
              <div className="text-right">
                <div className="flex items-center space-x-2 mb-1">
                  <span className={`text-xs px-2 py-1 rounded-full salon-transition-micro ${
                ...
                {nextAppointment && (
                  <div className="text-xs text-muted-foreground">
                    <p className="font-sans">Next: {nextAppointment?.time}</p>
                    <p className="truncate max-w-20">{nextAppointment?.customer}</p>
                  </div>
                )}

                {!nextAppointment && status === 'available' && (
                  <p className="text-xs text-muted-foreground font-sans">No upcoming appointments</p>
                )}
              </div>
            </div>
          );
      ...
      <div className="mt-4 pt-4 border-t border-border">
        <div className="grid grid-cols-2 gap-4 text-center">
          ...
          <div className="salon-transition-micro">
            <p className="text-2xl font-bold text-warning font-sans">
              {staffMembers?.filter(s => getAvailabilityStatus(s, selectedDate) === 'busy')?.length}
            </p>
            <p className="text-xs text-muted-foreground font-sans">Busy</p>
          </div>
        </div>
      </div>
    </div>
